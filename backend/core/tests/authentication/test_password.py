...
def password_authenticator():
    return PasswordAuthenticator(
        MagicMock(AuthenticationPresistence), "", MagicMock(CryptContext)
    )
...
def test_verify_password_success(password_authenticator):
    user_id, password = fake.pystr(), fake.pystr()
    expected_hash = fake.sha256()
    password_authenticator.authentication_presistence.get_password_hash = MagicMock(
    ...
    password_authenticator.authentication_presistence.get_password_hash.assert_called_once_with(
        user_id
    )
    password_authenticator.authentication_presistence.update_password_hash.assert_not_called()
...
def test_verify_password_fail_at_presistence(password_authenticator):
    user_id, password = fake.pystr(), fake.pystr()
    error = Err(fake.pystr())
    password_authenticator.authentication_presistence.get_password_hash = MagicMock(
        return_value=error
